...
func TestDeleteExpired(t *testing.T) {
	...
	t.Run("keepTTL", func(t *testing.T) {
		...
		t.Run("must forever when DefaultKeepTTL=false(default)", func(t *testing.T) {
			oldDefaultKeepTTL := DefaultKeepTTL
			defer func() {
				DefaultKeepTTL = oldDefaultKeepTTL
			}()
			DefaultKeepTTL = false // (default)
			testCase(t, 1)
		})
		t.Run("must expired when DefaultKeepTTL=true", func(t *testing.T) {
			oldDefaultKeepTTL := DefaultKeepTTL
			defer func() {
				DefaultKeepTTL = oldDefaultKeepTTL
			}()
